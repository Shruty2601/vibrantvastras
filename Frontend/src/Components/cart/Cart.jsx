...
async function initiatePayment(data){
  ...
  if(res){
   ...
   console.log(typeof(data.orderAmount)) 
    paymentOrder(data.orderAmount).then(res=>{
      ...
      if(res.message=='CREATED'){
        ...
        var options = {
          ...
          "prefill": {
              "name": "CSD Ecom Project",
              "email": "csdgroup1@gmail.com",
...
const CartItemsHtml=()=>{
  return(
    ...
    <Container>
    <h1>Cart Items({cart.item.length})</h1>
      <div className='mt-3'>
            {cart.item.map((cartItem,index)=>(
              ...
              <Card className='mt-2' border-0 shadow-sm color='light'>
                ...
                <CardText>
                  ...
                  <b> <h8 className='ms-3'>Price: {cartItem. totalproductprice}</h8></b>
            ...
            {console.log(cart.item.length)}
             { cart.item.length>0 ?(<Button onClick={()=>{setOrderProceed(true)}}  block color='primary' size='lg'>Click Here To Proceed</Button>):''}
            </Container>
      </div>
  </Container>
